...
// Register all joints interfaces    
    ...
    sub_left_wheel_delta_angle = nh_.subscribe("left_wheel_delta_angle", 10, &ROBOTHardwareInterface::callbackLeftWheelDeltaAngle, this);
    sub_right_wheel_delta_angle = nh_.subscribe("right_wheel_delta_angle", 10, &ROBOTHardwareInterface::callbackRightWheelDeltaAngle, this);
}
...
void ROBOTHardwareInterface::callbackLeftWheelDeltaAngle(const std_msgs::Int32::ConstPtr& msg)
{
	//left_motor.readBytes(rbuff,1);
    int x=msg->data;
    left_motor_pos+=angles::from_degrees((double)x);
...
void ROBOTHardwareInterface::callbackRightWheelDeltaAngle(const std_msgs::Int32::ConstPtr& msg)
{
    int x=msg->data;
    right_motor_pos+=angles::from_degrees((double)x);
    joint_position_[1]=right_motor_pos;
...
void ROBOTHardwareInterface::write(ros::Duration elapsed_time) {
    ...
    velocity=(float)angles::to_degrees(joint_velocity_command_[0]);
    
    //std::cout<<"Joint velocity: "<<joint_velocity_command_[0]<<std::endl;
	//wbuff[0]=velocity;
    //wbuff[1]=velocity >> 8;
    ...
    if(left_prev_cmd!=velocity)
    {
	    ...
	    ///msg.data = velocity;
	    //pub_left_wheel_speed.publish(msg);
	    //left_prev_cmd=velocity;
	    
	    odrive_ros_control::od_dual_control msg_dual_control;
            odrive_ros_control::od_control msg_control;    
	    msg_control.Vel_ax0 = velocity;	    
    }
    
    //velocity=(int)angles::to_degrees(joint_velocity_command_[1]);
    velocity=(float)angles::to_degrees(joint_velocity_command_[1]);
	//wbuff[0]=velocity;
    //wbuff[1]=velocity >> 8;
	//ROS_INFO("joint_velocity_command_[0]=%.2f velocity=%d  B1=%d B2=%d", joint_velocity_command_[0],velocity,wbuff[0],wbuff[1]);
    ...
    if(right_prev_cmd!=velocity)
    {
	    ...
	    std_msgs::Float32 msg;
	    msg.data = velocity;
	    pub_right_wheel_speed.publish(msg);
